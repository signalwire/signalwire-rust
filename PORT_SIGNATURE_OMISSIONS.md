...
## Subclass return-type collapse — Rust uses a tagged-union `Action`

Python returns specific RELAY action subclasses (`PlayAction`,
`RecordAction`, ...) where the corresponding Rust method returns the
parent type. Rust does not have inheritance, so it uses a single
`Action` struct with an internal kind tag and dispatches on that tag at
runtime. The contract is functionally equivalent — callers issue the
same RPC and observe the same fields.

(The REST half of this rationale no longer applies: `RestClient`'s
accessors return the CONCRETE generated resource types — `Chat`,
`Lookup`, `Recordings`, … — matching the reference, so those entries
were retired.)
...
signalwire.core.contexts.GatherInfo.add_question: Rust ``GatherInfo.add_question`` takes (key, question, question_type, confirm, prompt, functions, isolated) explicitly where Python takes (key, question, **kwargs) — the explicit tail is exactly the five kwargs Python's body reads (`kwargs.get` for type/confirm/prompt/functions/isolated), so no reachable field is lost
