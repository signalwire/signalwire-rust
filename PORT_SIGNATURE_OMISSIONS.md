...
signalwire.agents.bedrock.BedrockAgent.__init__: Rust BedrockAgent constructor takes a single ``options: BedrockOptions`` builder struct where Python takes 7 explicit kwargs (name, route, system_prompt, voice_id, temperature, top_p, max_tokens) — Rust uses an options-builder pattern (idiomatic for many-arg constructors, same as AgentBase/SWMLService/the prefab agents)
...
signalwire.core.swaig_function.SWAIGFunction.__init__: idiom: Rust `new(name, handler, description, parameters)` uses positional core args + fluent `secure()/required()/webhook_url()/...` builders for the optional kwargs; Python takes them all as constructor kwargs. Builder/options idiom.
...
signalwire.relay.call.StandaloneCollectAction.__init__: idiom: Rust `new(control_id, call_id, node_id)` builds the action from the wire correlation ids; Python constructs from a call + component. Rust newtype-over-Action idiom.
...
signalwire.rest._base.HttpClient.__init__: idiom: Rust `new(project_id, token, space, ...)` builds the client with a pluggable transport (UreqTransport/StubTransport); Python `(self)` reads a requests session. Transport-injection idiom.
...
signalwire.rest._base.SignalWireRestError.__init__: idiom: Rust `new(message, status_code, response_body)` typed-error constructor; Python `(self, status_code, body, url, method)` exception. Rust typed-error field shape (mirrors the REST error exemplar).
...
signalwire.relay.event.CallReceiveEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.CallStateEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.CallingErrorEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.CollectEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.ConferenceEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.ConnectEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.DenoiseEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.DetectEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.DialEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.EchoEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.FaxEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.HoldEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.MessageReceiveEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.MessageStateEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.PayEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.PlayEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.QueueEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.RecordEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.ReferEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.RelayEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.SendDigitsEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.StreamEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.TapEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.relay.event.TranscribeEvent.__init__: idiom: Rust typed event wrapper has no field-wise constructor — built via `from_payload`; the Python dataclass `__init__` field list is exposed as `&self` accessors delegating to the wrapped `RelayEvent`. Wrapper idiom.
...
signalwire.core.agent.prompt.manager.PromptManager.__init__: idiom: Python's extracted `PromptManager(agent)` delegate wraps the agent; Rust folds the prompt surface directly onto AgentBase, so the projected PromptManager carries the reference method set but no `agent`-bound constructor. Delegate-folded-onto-AgentBase idiom.
signalwire.core.agent.tools.registry.ToolRegistry.__init__: idiom: Python's extracted `ToolRegistry(agent)` delegate wraps the agent; Rust folds the tool registry onto AgentBase/Service, so the projected ToolRegistry carries the reference method set but no `agent`-bound constructor. Delegate-folded idiom.
...
signalwire.core.security_config.SecurityConfig.__init__: idiom: Rust `SecurityConfig::new()` builds a defaulted config then loads via `load_from_env()` (fluent); Python `__init__(config_file, service_name)` takes the source args in the constructor. Default-then-load idiom.
signalwire.web.web_service.WebService.__init__: idiom: Rust `WebService::new(options)` bundles the ten Python constructor kwargs (`port/directories/basic_auth/config_file/enable_directory_browsing/allowed_extensions/blocked_extensions/max_file_size/enable_cors`) into one options struct; Python explodes them. options-struct ≡ kwargs idiom.
...
signalwire.core.skill_base.SkillBase.__init__: idiom: `SkillBase` is a Rust trait (no constructor); the reference records a dataclass `__init__(agent, params)`. Rust skills are built by their concrete `impl` + the factory registry, not a trait constructor. Trait-no-ctor idiom.
...
signalwire.core.skill_base.SkillBase.logger: idiom: Python `@property logger` exposes a per-skill logger; Rust skills log via the crate `Logger` facade (no per-instance `logger` accessor method). Facade-logger idiom.
...
signalwire.relay.client.RelayError.__init__: idiom: Rust `RelayError` is a typed error enum (constructed by variant, not `new(code, message)`); the Python reference records an exception `__init__(code, message)`. Rust typed-error-enum idiom.
...
signalwire.rest._base.BaseResource.__init__: idiom: `BaseResource` is the Rust generated-layer base (the `_base` surface is represented by the legacy `rest::CrudResource`); the projected reference `__init__(http, base_path)` carries the reference param shape while the Rust bases compose via the generated resource tree. Generated-base idiom.
...
signalwire.rest._base.SignalWireRestTransportError.__init__: rust-error-cause-wrap (plan 1.3b) — the Rust transport error is the SAME SignalWireRestError struct (folded via an is_transport() discriminator, status_code() 0 == the reference's status_code=None), projected onto this reference class via SignalWireRestError's plain `new` constructor shape for name/identity parity (SURFACE-DIFF). Rust's actual transport constructor is `SignalWireRestError::transport(message, url, method, cause)`, which takes one EXTRA `cause: E: std::error::Error` arg beyond the reference's (body, url, method) so the underlying transport error is preserved as this error's `source()` — the Rust equivalent of Python's `raise SignalWireRestTransportError(...) from exc`, which Python expresses via `__cause__` (not a constructor param). message/url/method roles all map through; `cause.to_string()` fills the body role when no body is given. Mirrors the Go port's identical `NewSignalWireRestTransportError(cause, body, url, method)` divergence for this same plan.
...
signalwire.rest._base.ReadResource.paginate: idiom: Rust `paginate(&self, params: &HashMap<String,String>) -> PaginatedIterator`; the reference gained a keyword-only `request_options` (`paginate(self, *, request_options=None, **params)` — the enumerator drops `**params`, so the reference records `(self, request_options)`). Rust takes the query `params` positionally (same as `list`) and threads the per-request override through the sibling `paginate_with_options`; the client-default RequestOptions still applies to plain `paginate`. Same `*_with_options` idiom as HttpClient.put/patch and the base CRUD verbs.
signalwire.rest._request_options.RequestOptions.__init__: idiom: Rust `RequestOptions::new()` builds an all-unset options object and sets fields via chained builder setters (timeout/retries/retry_on_status/retry_backoff/abort_signal); Python's dataclass `__init__` takes all five as optional kwargs. Builder-vs-dataclass construction — same fields, Rust idiom.
...
signalwire.rest.client.RestClient.__init__: idiom: Rust `RestClient::new(project_id, token, space)` / `with_base_url(...)`; the reference constructor gained a `request_options` param. Rust threads the client-default RequestOptions through the sibling `with_base_url_and_options` constructor (no default-arg overloading) — the base constructors preserve their existing callers.

## AI Chat client (signalwire.ai_chat.client) — options-object + struct-literal idiom

signalwire.ai_chat.client.AIChatClient.__init__: idiom: Rust constructs the client via the fluent `AIChatClient::builder()` associated fn (folded onto `__init__` in the surface); its chainable setters (project/token/space/url/read_idle_timeout_secs) carry the reference `__init__`'s (project, token, space, url, session) keyword args — there is no single keyword-arg constructor with a matching param list (no default-arg overloading), and the connection-lifetime knob is Rust's `read_idle_timeout_secs` rather than the reference's `session`. Same builder-idiom construction as `RestClient.__init__`. The emitted wire/auth is identical (AI-CHAT gate verified).
signalwire.ai_chat.client.AIChatClient.create_conversation: Rust ``create_conversation(conversation_id, options: CreateOptions)`` collapses Python's (config_url, user_message, timeout, user_metadata, reinit) keyword args into a single ``CreateOptions`` builder struct — the Rust options-object idiom for optional/many-arg methods. The wire params emitted are identical (AI-CHAT wire gate verified).
signalwire.ai_chat.client.AIChatClient.chat: Rust ``chat(conversation_id, message, options: ChatOptions)`` collapses Python's (role, config_url, user_metadata, timeout, reinit) keyword args into a single ``ChatOptions`` builder struct — the Rust options-object idiom. The wire params emitted are identical (AI-CHAT wire gate verified).
signalwire.ai_chat.client.ChatLog.__init__: Rust ``ChatLog`` is a plain public-field struct (struct-literal / field access); Python's dataclass ``__init__`` has no Rust analogue.
signalwire.ai_chat.client.ChatResponse.__init__: Rust ``ChatResponse`` is a plain public-field struct (struct-literal / field access); Python's dataclass ``__init__`` has no Rust analogue.
signalwire.ai_chat.client.ConversationInfo.__init__: Rust ``ConversationInfo`` is a plain public-field struct (struct-literal / field access); Python's dataclass ``__init__`` has no Rust analogue.
