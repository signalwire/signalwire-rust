...
signalwire.core.function_result.FunctionResult.toggle_functions: Rust takes ``list<Value>`` (a ``Vec<serde_json::Value>``) where Python takes ``list<dict<string,any>>`` — both pass the ordered list through verbatim (preserving order + arbitrary keys/values), so the EMISSION is byte-identical (verified by diff_port_emission.py ``toggle_functions``); same idiom as ``add_dynamic_hints``. (A prior ``dict<string,bool>`` signature lost ordering and forbade extra keys — that latent emission bug was fixed.)
