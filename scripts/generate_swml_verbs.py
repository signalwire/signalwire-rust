    ...
    for rs_name, props, tdesc, spec_name in types:
        src += (
            ...
            + GR.emit_methodless_struct(
                rs_name, props, tdesc, "generate_swml_verbs.py", schema_name=spec_name
            )
            + "\n"
        )
...
def build_sidecar(psdk: Path) -> str:
    ...
    types = build_types(psdk)
    classes = {
        rs_name: GR.gen_payload_accessors(props, spec_name)
        for rs_name, props, _, spec_name in types
    }
    payload = {
        "_comment": (
            "Generated by scripts/generate_swml_verbs.py; DO NOT EDIT. "
