...
def build_types(psdk: Path) -> list[tuple[str, dict, str, str | None]]:
    ...
    def add(rs_name: str, props: dict, desc: str, spec_name: str | None) -> None:
        if rs_name in seen:
            return
        seen.add(rs_name)
        out.append((rs_name, props, desc, spec_name))
    ...
    for raw_name, node in defs.items():
        ...
        add(GR.type_name(raw_name), node.get("properties") or {},
            f"schema.json $defs schema {raw_name!r}", raw_name)
    ...
    if sm:
        for ref in sm.get("anyOf") or []:
            ...
            add(GR.type_name(_pascal(verb) + "Config"), props,
                f"flattened SWMLMethod verb {verb!r} config", None)
...
def build_sidecar(psdk: Path) -> str:
    ...
    types = build_types(psdk)
    classes = {rs_name: GR.gen_payload_accessors(props, spec_name)
               for rs_name, props, _, spec_name in types}
    payload = {
    ...
    if args.check:
        ...
        for fn, src in outs.items():
            ...
            if not p.is_file() or p.read_text() != src:
                stale.append(str(p))
        if stale:
            sys.stderr.write(f"GEN-FRESH FAIL: {len(stale)} generated SWML-verb file(s) stale:\n")
            for s in stale:
