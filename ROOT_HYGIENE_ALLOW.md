...
- rust-toolchain.toml — toolchain PIN (`channel = "1.96.0"`, `components = ["rustfmt", "clippy"]`) read at exactly this root path by TWO readers: rustup itself, which walks UP from the invocation CWD and so honours the file only at or above it (verified empirically: with a copy at `eng/rust-toolchain.toml`, `rustup show active-toolchain` from the repo root still reported `overridden by '<repo>/rust-toolchain.toml'` and the `eng/` copy applied only when the CWD was inside `eng/`); and scripts/_env.sh:58, which reads `"$REPO/rust-toolchain.toml"` by explicit path to derive `SW_RUST_TOOLCHAIN` and hard-exits `FATAL: rust-toolchain.toml is missing or has no [toolchain] channel.` if it is absent. rustup has no relocation flag, so the path IS the contract and the gate's "move to eng/" advice would silently UN-PIN the compiler. That pin is load-bearing rather than cosmetic: Cargo.toml:67-69 denies `clippy::all` and `pedantic` at `priority = -1` and scripts/run-lint.sh:29 adds `-D warnings`, so an unpinned toolchain makes every new stable clippy release an instant build failure with nothing in the diff to explain it, which is the failure 079a919 added the file to prevent (`rust-version = "1.88"` is an MSRV floor and constrains nothing above itself). Governs local runs, CI, and editors alike; the four workflows pin `dtolnay/rust-toolchain@1.96.0` in lockstep with it (mike@signalwire.com, 2026-08-06)
