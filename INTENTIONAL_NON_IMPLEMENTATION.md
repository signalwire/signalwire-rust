...
- src/swml/service.rs:513 — handle_swml_request returns the rendered SWML doc identically for GET and POST per Python's SWMLService base behavior; AgentBase wraps for dynamic-config (request-body-driven rendering) but the SWMLService base correctly ignores `_method` / `_request_data` / `_headers` and serves the static doc. Category 4 — extension-point default with the documented behavior.
