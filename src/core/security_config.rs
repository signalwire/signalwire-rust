...
impl SecurityConfig {
    ...
    pub fn load_from_env(&mut self) {
        ...
        self.ssl_enabled = env::var(SSL_ENABLED).map(|v| truthy(&v)).unwrap_or(false);
        ...
        self.use_hsts = match env::var(USE_HSTS) {
            Ok(v) if !v.is_empty() => v.to_ascii_lowercase() != "false",
            _ => true,
        };
    ...
    pub fn get_basic_auth(&mut self) -> (String, String) {
        ...
        let password = match &self.basic_auth_password {
            Some(p) => p.clone(),
            None => {
                ...
                if !self.basic_auth_autogen_warned {
                    ...
                    // password will 401 external callers.
                    log::warn!(
                        "basic_auth_password_autogenerated for user {username}: set \
                         SWML_BASIC_AUTH_USER / SWML_BASIC_AUTH_PASSWORD to suppress"
                    );
                }
                p
            }
