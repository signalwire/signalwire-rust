...
impl Service {
    pub fn new(options: ServiceOptions) -> Self {
        ...
        // and changes on every restart. PRINT THE GENERATED CREDENTIALS
        // ONCE to stderr so a developer can actually authenticate on the
        // first run (r5 deep_dogfood: rust generated a password and never
        // surfaced it, so there was no way to hit an authed endpoint). This
        // is a first-run dev-usability affordance; set SWML_BASIC_AUTH_* to
        // suppress it and pin a stable password.
        if password_auto_generated {
            logger.warn(&format!(
                "basic_auth_password_autogenerated. No SWML_BASIC_AUTH_PASSWORD in \
                 the environment and none passed via ServiceOptions, so the SDK \
                 generated a random password that exists only in this process and \
                 changes on every restart. Generated basic-auth credentials for \
                 this run:\n    username: {basic_auth_user}\n    password: {basic_auth_password}\n\
                 Use these to authenticate, or set SWML_BASIC_AUTH_USER and \
                 SWML_BASIC_AUTH_PASSWORD in your environment (or pass \
                 basic_auth_user / basic_auth_password in ServiceOptions / \
                 AgentOptions) to pin stable credentials."
            ));
        }
