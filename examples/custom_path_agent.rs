...
fn main() {
    ...
    agent.set_dynamic_config_callback(Box::new(|query_params, _body_params, _headers, agent| {
        ...
        agent.prompt_add_section(
            "Personalization",
            &format!("The user's name is {user_name}. They're interested in discussing {topic}."),
            vec![],
        );
