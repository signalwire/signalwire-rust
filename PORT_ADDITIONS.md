...
### AI Chat client — Rust options-builder / accessor idiom (signalwire.ai_chat.client)

The AI Chat client's core methods and data models (create_conversation/chat/end/delete/log/summarize; ChatLog/ChatResponse/ConversationInfo) MATCH the Python reference. The port-only symbols below are the Rust idiom that replaces Python's keyword-argument constructor and optional method kwargs: an options-builder (`AIChatClientBuilder`) and typed options structs (`ChatOptions`/`CreateOptions`/`SummarizeOptions`), plus read-only field accessors. They add no wire behavior — the emitted params are identical (AI-CHAT-gate verified).

signalwire.ai_chat.client.AIChatClient.builder: Rust options-builder idiom: `AIChatClient::builder()` returns `AIChatClientBuilder`, whose chainable setters (project/token/space/url/read_idle_timeout_secs) replace Python's keyword-argument `__init__`. No Python twin — Python constructs via `__init__`.
signalwire.ai_chat.client.AIChatClient.project: Rust read-only accessor exposing a client field; Python reads the attribute directly (`client.url` / `client.project`). Idiomatic Rust getter, no behavioral surface change.
signalwire.ai_chat.client.AIChatClient.url: Rust read-only accessor exposing a client field; Python reads the attribute directly (`client.url` / `client.project`). Idiomatic Rust getter, no behavioral surface change.
signalwire.ai_chat.client.AIChatClientBuilder: Rust options-builder idiom: `AIChatClient::builder()` returns `AIChatClientBuilder`, whose chainable setters (project/token/space/url/read_idle_timeout_secs) replace Python's keyword-argument `__init__`. No Python twin — Python constructs via `__init__`.
signalwire.ai_chat.client.AIChatClientBuilder.build: Rust options-builder idiom: `AIChatClient::builder()` returns `AIChatClientBuilder`, whose chainable setters (project/token/space/url/read_idle_timeout_secs) replace Python's keyword-argument `__init__`. No Python twin — Python constructs via `__init__`.
signalwire.ai_chat.client.AIChatClientBuilder.project: Rust options-builder idiom: `AIChatClient::builder()` returns `AIChatClientBuilder`, whose chainable setters (project/token/space/url/read_idle_timeout_secs) replace Python's keyword-argument `__init__`. No Python twin — Python constructs via `__init__`.
signalwire.ai_chat.client.AIChatClientBuilder.space: Rust options-builder idiom: `AIChatClient::builder()` returns `AIChatClientBuilder`, whose chainable setters (project/token/space/url/read_idle_timeout_secs) replace Python's keyword-argument `__init__`. No Python twin — Python constructs via `__init__`.
signalwire.ai_chat.client.AIChatClientBuilder.token: Rust options-builder idiom: `AIChatClient::builder()` returns `AIChatClientBuilder`, whose chainable setters (project/token/space/url/read_idle_timeout_secs) replace Python's keyword-argument `__init__`. No Python twin — Python constructs via `__init__`.
signalwire.ai_chat.client.AIChatClientBuilder.url: Rust options-builder idiom: `AIChatClient::builder()` returns `AIChatClientBuilder`, whose chainable setters (project/token/space/url/read_idle_timeout_secs) replace Python's keyword-argument `__init__`. No Python twin — Python constructs via `__init__`.
signalwire.ai_chat.client.ChatOptions: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.ChatOptions.config_url: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.ChatOptions.role: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.ChatOptions.user_metadata: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.CreateOptions: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.CreateOptions.__init__: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.CreateOptions.user_message: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.CreateOptions.user_metadata: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.SummarizeOptions: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.ai_chat.client.SummarizeOptions.summary_prompt: Rust options-object idiom: a builder struct collapsing the corresponding Python method's optional keyword arguments into one typed value (the Rust shape for optional/many-arg methods). The fields map 1:1 onto the Python kwargs and emit identical wire params (AI-CHAT-gate verified).
signalwire.AIChatClient: Top-level re-export `signalwire::AIChatClient` (mirrors Python's `from signalwire import AIChatClient`). Present in python_signatures.json; the lib.rs `pub use` surfaces it flat at the crate root for `use signalwire::*` parity.
